...
def savehash(addr):
	...
	etat = lll[0].etat
	if etat == 1:
		return "votre checksum a bien été enregistré c le txidlll        [0].txid
	#add another field to database which contains 0 or txid
	#if etat=1 print thetxid else print still waiting for transaction
	else:
		return "toujours en attente d'envoie de 0.0005 BTC à cette addresse " + addr + " pour pouvoir enregistrer votre checksum"
